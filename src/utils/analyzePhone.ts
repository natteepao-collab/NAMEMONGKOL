...
export const analyzePhone = async (phoneNumber: string): Promise<PhoneAnalysisResult | null> => {
    ...
    for (let i = 0; i < meaningfulPart.length - 1; i++) {
        ...
        const impact = 20; // 5 pairs to max out (100%)

        const updateStat = (key: keyof typeof stats) => {
            if (isGood) stats[key].pos += impact;
            if (isBad) stats[key].neg += impact;
        };
    ...
    return {
        ...
        grade,
        stats,
        prediction: getOverallPrediction(grade)
    };
};

const getOverallPrediction = (grade: string): string => {
    switch (grade) {
        case 'A+': return 'เบอร์มงคลเกรดพรีเมียม ส่งเสริมชีวิตรอบด้านอย่างดีเยี่ยม';
        case 'A': return 'เบอร์มงคลระดับสูง ดีเยี่ยมในหลายๆ ด้าน';
        case 'B+': return 'เบอร์ดี ส่งเสริมความก้าวหน้า';
        case 'B': return 'เบอร์ระดับกลาง พอใช้ได้ มีทั้งดีและเสียปนกัน';
        case 'C': return 'เบอร์ทั่วไป ควรพิจารณาเปลี่ยนหากต้องการเสริมดวง';
        case 'D': return 'เบอร์ควรเลี่ยง มีเลขเสียฉุดดวงชะตา';
